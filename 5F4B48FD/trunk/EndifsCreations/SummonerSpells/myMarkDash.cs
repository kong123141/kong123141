...
namespace EndifsCreations.SummonerSpells
{
    class myMarkDash
    {
        ...
        void OnDraw(EventArgs args)
        {
            ...
            if (tools.Item("msb_draw").GetValue<bool>())
            {
                if (IsReady)
                {
                    Render.Circle.DrawCircle(ObjectManager.Player.Position, 1600, Color.Yellow, 7);
                }
                if (HaveSnowball && Snowball.Instance.Name.Equals("snowballfollowupcast"))
                {
                    Drawing.DrawText(ObjectManager.Player.HPBarPosition.X + 10, ObjectManager.Player.HPBarPosition.Y - 25, Color.Cyan, "Marked");
                }
            }
        }
    }
