...
namespace EndifsCreations.Plugins
{
    class Cassiopeia : PluginData
    {
        ...
        private void Combo()
        {
            ...
            var UseQ = config.Item("EC.Cassiopeia.Combo.Q").GetValue<bool>();
            var UseW = config.Item("EC.Cassiopeia.Combo.W").GetValue<bool>();
            var UseE = config.Item("EC.Cassiopeia.Combo.E").GetValue<bool>();
            var UseR = config.Item("EC.Cassiopeia.Combo.R").GetValue<bool>();
        ...
        protected override void OnUpdate(EventArgs args)
        {
            ...
            switch (myOrbwalker.ActiveMode)
            {
                ...
                case myOrbwalker.OrbwalkingMode.LaneClear:
                    if (myUtility.EnoughMana(config.Item("EC.Cassiopeia.Farm.ManaPercent").GetValue<Slider>().Value))
                    {
                        var list = MinionManager.GetMinions(Player.ServerPosition, E.Range).Where(x => x.HasBuffOfType(BuffType.Poison)).ToList();
                        if (config.Item("EC.Cassiopeia.Farm.W").GetValue<bool>() && W.IsReady() && list.Count <= 0)
                        {
                            myFarmManager.LaneCircular(W, W.Range, 125);
                        }
                        if (config.Item("EC.Cassiopeia.Farm.E").GetValue<bool>() && E.IsReady())
                        {
        ...
        protected override void OnDraw(EventArgs args)
        {
            ...
            if (config.Item("EC.Cassiopeia.Draw.Q").GetValue<bool>() && Q.Level > 0)
            {
                Render.Circle.DrawCircle(Player.Position, Q.Range, Color.White);
            }
            if (config.Item("EC.Cassiopeia.Draw.W").GetValue<bool>() && W.Level > 0)
            {
                Render.Circle.DrawCircle(Player.Position, W.Range, Color.White);
            }
            if (config.Item("EC.Cassiopeia.Draw.E").GetValue<bool>() && E.Level > 0)
            {
                Render.Circle.DrawCircle(Player.Position, E.Range, Color.White);
            }
            if (config.Item("EC.Cassiopeia.Draw.R").GetValue<bool>() && R.Level > 0)
            {
