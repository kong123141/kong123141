...
namespace 花边_花式多合一.Core
{
    class SharpExperience
    {
        ...
        internal static void Game_OnGameLoad(EventArgs args)
        {
            try
            {
                if (!InitializeMenu.Menu.Item("SharedExperienceEnable").GetValue<bool>()) return;
            ...
            {
                Console.WriteLine("SharpExperience error occurred: '{0}'", ex);
            }
        }
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            foreach (Obj_AI_Hero hero in ObjectManager.Get<Obj_AI_Hero>())
            {
                ...
                int x = InitializeMenu.Menu.Item("SharedExperienceposX").GetValue<Slider>().Value;
                int y = InitializeMenu.Menu.Item("SharedExperienceposY").GetValue<Slider>().Value + c * 30;

                if ((Environment.TickCount - TimeChangedExp[i]) < 5000 && InvisibleCount[i] > 0)
                {
                    if (InitializeMenu.Menu.Item("SharedExperiencedrawPredictionCircle").GetValue<bool>()) Drawing.DrawCircle(LastMinionPosition[i], 1500, Color.Red);
                ...
                {
                    if (InitializeMenu.Menu.Item("SharedExperiencedrawchampionlist").GetValue<bool>())
                    {
                        Drawing.DrawLine(
                ...
                if (!InitializeMenu.Menu.Item("SharedExperienceshowEnemies").GetValue<bool>()) continue;

                int textXOffset = InitializeMenu.Menu.Item("SharedExperiencepositionX").GetValue<Slider>().Value;
                int textYOffset = InitializeMenu.Menu.Item("SharedExperiencepositionY").GetValue<Slider>().Value;
                ...
                if (SharingCount[i] > 0)
                {
                    ...
                    if (!InitializeMenu.Menu.Item("SharedExperienceonlyShowInv").GetValue<bool>() && InvisibleCount[i] == 0)
                    {
        ...
        private static void OnGameUpdate(EventArgs args)
        {
            ...
            foreach (Obj_AI_Hero hero in ObjectManager.Get<Obj_AI_Hero>())
            {
                ...
                if (InvisibleCount[i] > 0)
                {
                    Cor[i] = InitializeMenu.Menu.Item("SharedExperienceinvColor").GetValue<Color>();
