...
namespace 花边_花式多合一.Core
{
    ...
    {
        public List<Obj_AI_Hero> TrackList = new List<Obj_AI_Hero>();
        public static Geometry.Polygon _myPoly;
        public static Geometry.Polygon.Circle futurePosCircle;

        public damageHandler(List<Obj_AI_Hero> allyList)
        {
            TrackList = allyList;
        ...
        void Obj_AI_Hero_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            else //if its a Spell
            {
                ...
                foreach (var spellInfo in
                    ActivatorSpellDatabase.Spells.Where(spellInfo => spellInfo.spellName.Equals(args.SData.Name)))
                {
        ...
        public Geometry.Polygon getSkillPolygon()
        {
            return _myPoly;
        }

        public Geometry.Polygon.Circle getPredictedPos()
        {
            return futurePosCircle;
        }

        public static List<ActivatorAttacked> attacksLog = new List<ActivatorAttacked>();

        public void Add(Obj_AI_Hero ally, double damage, double timeToHit)
        {
            ActivatorAttacked attack = new ActivatorAttacked(ally, damage, new Stopwatch(), timeToHit);
            attacksLog.Add(attack);
            Utility.DelayAction.Add((int)timeToHit, () => attacksLog.Remove(attack));
            attacksLog = attacksLog.OrderBy(Hit => Hit.timeToHit - Hit.timer.ElapsedMilliseconds).ToList();
            Console.WriteLine("Damage " + damage + " Time " + timeToHit);
        }

        public double GetPredictedDamage(Obj_AI_Hero ally)
        {
            double sum = 0;
            foreach (var attack in attacksLog)
            {
                sum += attack.Damage;
            }
            return sum;
        ...
        public List<ActivatorAttacked> getAttacks()
        {
            return attacksLog;
        }

        public double GetFirstAttackTime()
        {
            if (attacksLog.First() == null)
                return 0;
            return attacksLog.First().timeToHit - attacksLog.First().timer.ElapsedMilliseconds;
        }

    }

    public static class ActivatorSpellDatabase
    {
        public static List<ActivatorSpellData> Spells = new List<ActivatorSpellData>();

        static ActivatorSpellDatabase()
        {
            #region AllChampions

            Spells.Add(
            new ActivatorSpellData
            {
    ...
    public class ActivatorSpellData
    {
    ...
    public class ActivatorAttacked
    {
        public Obj_AI_Hero Ally { get; set; }
        public double Damage { get; set; }
        public Stopwatch timer { get; set; }
        public double timeToHit { get; set; }

        public ActivatorAttacked(Obj_AI_Hero ally, double damage, Stopwatch timer, double timeToHit)
        {
            Ally = ally;
            Damage = damage;
            this.timer = timer;
