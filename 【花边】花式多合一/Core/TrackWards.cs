...
namespace 花边_花式多合一.Core
{
    class WardTrackerInfo
    {
            ...
            float timestamp = 0)
        {
            ...
            this.timestamp = timestamp == 0 ? DelayAction.TickCount : timestamp;
            this.endTime = this.timestamp + wardData.duration;
        }
    }

    internal class TrackWards
    {
        ...
        internal static void Game_OnGameLoad(EventArgs args)
        {
            ...
            {
                Console.WriteLine("TrackWard error occurred: '{0}'", ex);
            }
        }
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            foreach (var ward in wards)
            {
                ...
                if (wardPos.IsOnScreen() && ward.endTime > DelayAction.TickCount)
                {
                    var wardScreenPos = Drawing.WorldToScreen(wardPos);
                    var timeStr = TextUtils.FormatTime((ward.endTime - DelayAction.TickCount) / 1000f);
                    var tSize = tStrSize;
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (DelayAction.TickCount > lastCheckExpiredWards)
            {
                CheckExpiredWards();
                lastCheckExpiredWards = DelayAction.TickCount + 500;
            }
        }
        ...
        private static void CheckExpiredWards()
        {
            foreach (var ward in wards)
            {
                if (DelayAction.TickCount > ward.endTime)
                {
        ...
        private static void Game_OnCreateObj(GameObject sender, EventArgs args)
        {
            ...
            {
                var timestamp = DelayAction.TickCount - (obj.MaxMana - obj.Mana) * 1000;
                ...
                DelayAction.Add(500, () =>
                {
                    ...
                    {
                        timestamp = DelayAction.TickCount - (obj.MaxMana - obj.Mana) * 1000;
                ...
                if (missile.SData.Name.ToLower() == "itemplacementmissile")// && !missile.SpellCaster.IsVisible)
                {
                    ...
                    foreach (var ward in wards) //check for visible
                    {
                        if (ward.position.To2D().Distance(pos) < 750
                            && DelayAction.TickCount - ward.timestamp < 50)
                        {
                            return;
                        }
                    }
                    ...
                    DelayAction.Add(100, () =>
                    {
                        ...
                        {
                            if (ward.position.To2D().Distance(pos) < 750
                                && DelayAction.TickCount - ward.timestamp < 125)
                            {
                                return;
                            }
                        }
        ...
        private static void Obj_AI_Hero_OnProcessSpellCast(Obj_AI_Base hero, GameObjectProcessSpellCastEventArgs args)
        {
            if (!InitializeMenu.Menu.Item("TrackEnemyWards").GetValue<bool>())
            {
                return;
                    ...
                    DelayAction.Add(50, () =>
                    {
                        foreach (var ward in wards)
                        {
                            if (ward.position.To2D().Distance(pos) < 50
                                && DelayAction.TickCount - ward.timestamp < 100)
                            {
                                return;
                            }
                        }
