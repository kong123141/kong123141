...
namespace HeavenStrikeLuLu
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Game.PrintChat("Welcome to Heaven Strike Lulu");
        }
        ...
        public static void Auto()
        {
            ...
            // case KS with EQ
            if (_q.IsReady() && _e.IsReady() && Player.Mana >= _q.Instance.ManaCost + _e.Instance.ManaCost && _menu.Item("QEA").GetValue<bool>())
            {
                // EQ on same target
        ...
        public static void Harass()
        {
            if (Player.Mana * 100 / Player.MaxMana >= _menu.Item("ManaH").GetValue<Slider>().Value)
            {
                ...
                // Harass EQ
                if (_q.IsReady() && _e.IsReady() && Player.Mana >= _q.Instance.ManaCost + _e.Instance.ManaCost && _menu.Item("QEH").GetValue<bool>() && _menu.Item("QH").GetValue<bool>())
                {
                    var hero = TargetSelector.GetTarget(_q.Range + _e.Range,TargetSelector.DamageType.Magical);
                    if (hero != null && hero.IsValidTarget() && !hero.IsValidTarget(_q.Range))
                    {
