    ...
    public sealed class MenuCustomizer : Menu
    {
        ...
        private void AddContainer(Menu menu)
        {
            this.ContainerHeight =
                menu.Add(new MenuSlider("containerheight", "菜单高度", MenuSettings.ContainerHeight, 15, 50));
            this.FontHeight =
        ...
        {
            var customizeMenu = this.Add(new Menu("customize", "自设界面"));
            this.AddPosition(customizeMenu);
            this.AddContainer(customizeMenu);
            customizeMenu.Add(new MenuButton("apply", "启动修改", "启动") { Action = this.ApplyChanges });
            customizeMenu.Add(
                new MenuButton("reset", "重置默认", "重置")
                    {
                        Action = () =>
                            {
        ...
        {
            this.LockPosition = this.Add(new MenuBool("lock", "锁定菜单位置"));
        }
