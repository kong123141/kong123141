...
namespace iYasuo.utils
{
    internal class AssassinManager
    {
        ...
        private static void ClearAssassinList()
        {
            ...
            {
                Program.Menu.Item("Assassin" + enemy.ChampionName).SetValue(false);
            }
        }
        ...
        {
            if (Program.Menu.Item("AssassinReset").GetValue<KeyBind>().Active && args.Msg == 257)
            {
                ClearAssassinList();
                Game.PrintChat(
            ...
            if (Program.Menu.Item("AssassinSetClick").GetValue<bool>())
            {
                    ...
                    select enemy)
                {
                    ...
                    {
                        var xSelect = Program.Menu.Item("AssassinSelectOption").GetValue<StringList>().SelectedIndex;
                        switch (xSelect)
                        {
                            case 0:
                                ClearAssassinList();
                                Program.Menu.Item("Assassin" + objAiHero.ChampionName).SetValue(true);
                                Game.PrintChat(
                                    string.Format(
                                ...
                                break;
                            case 1:
                                var menuStatus = Program.Menu.Item("Assassin" + objAiHero.ChampionName).GetValue<bool>();
                                Program.Menu.Item("Assassin" + objAiHero.ChampionName).SetValue(!menuStatus);
                                Game.PrintChat(
                                    string.Format(
        ...
        {
            if (!Program.Menu.Item("AssassinActive").GetValue<bool>())
            {
                return;
            }
            if (Program.Menu.Item("DrawStatus").GetValue<bool>())
            {
                ...
                {
                    var xValue = Program.Menu.Item("Assassin" + objAiHeroes[i].ChampionName).GetValue<bool>();
                    DrawTextBold(
            ...
            var drawSearch = Program.Menu.Item("DrawSearch").GetValue<Circle>();
            var drawActive = Program.Menu.Item("DrawActive").GetValue<Circle>();
            var drawNearest = Program.Menu.Item("DrawNearest").GetValue<Circle>();
            var drawSearchRange = Program.Menu.Item("AssassinSearchRange").GetValue<Slider>().Value;
            ...
            foreach (var enemy in
                ObjectManager.Get<Obj_AI_Hero>()
                    ...
                    .Where(
                        enemy =>
                            enemy.IsVisible && Program.Menu.Item("Assassin" + enemy.ChampionName) != null &&
                            !enemy.IsDead)
                    .Where(enemy => Program.Menu.Item("Assassin" + enemy.ChampionName).GetValue<bool>()))
            {
