namespace Pluging
{
    ...
    public class SharedExperience
    {
        ...
        public SharedExperience(Menu mainMenu)
        {
            Menu = mainMenu;


            Menu SharedExperienceMenu = new Menu("[FL] 经验分流", "SharedExperience");

            SharedExperienceMenu.AddItem(new MenuItem("showEnemies", "Draw Text On Enemy").SetValue(true));
            SharedExperienceMenu.AddItem(new MenuItem("onlyShowInv", "Only Draw Text When Not Visible Enemies").SetValue(false));
            SharedExperienceMenu.AddItem(new MenuItem("drawPredictionCircle", "Draw Prediction Circle").SetValue(true));
            SharedExperienceMenu.AddItem(new MenuItem("invColor", "Text Color When Not Visible Enemies").SetValue(Color.FromArgb(255, 245, 25, 25)));
            SharedExperienceMenu.AddItem(new MenuItem("positionX", "OnEnemy Text Position X").SetValue(new Slider(142, -100, 200)));
            SharedExperienceMenu.AddItem(new MenuItem("positionY", "OnEnemy Text Position Y").SetValue(new Slider(21, -100, 100)));

            SharedExperienceMenu.AddSubMenu(new Menu("Champion List", "Champion List"));
            SharedExperienceMenu.SubMenu("Champion List").AddItem(new MenuItem("drawchampionlist", "Draw Champion List").SetValue(false));
            SharedExperienceMenu.SubMenu("Champion List").AddItem(new MenuItem("posX", "Champions List Pos X").SetValue(new Slider(Drawing.Width / 2, 0, Drawing.Width)));
            SharedExperienceMenu.SubMenu("Champion List").AddItem(new MenuItem("posY", "Champions List Pos Y").SetValue(new Slider(Drawing.Height / 2, 0, Drawing.Height)));

            Menu.AddSubMenu(SharedExperienceMenu);
        ...
        private void OnDraw(EventArgs args)
        {
            try
            {
                ...
                foreach (Obj_AI_Hero hero in ObjectManager.Get<Obj_AI_Hero>())
                {
                    ...
                    {
                        if (Menu.Item("drawPredictionCircle").GetValue<bool>())
                    ...
                    if (IsNearMe[i] == 1 && !hero.IsDead)
                    {
                        ...
                        {
                            Drawing.DrawLine(new Vector2(x - 4.5f, y - 5), new Vector2(x + 85, y - 5), 3, VisibleColor[i]);

                            Drawing.DrawLine(new Vector2(x + 85, y - 4.5f), new Vector2(x + 85, y + 21), 3, VisibleColor[i]);

                            Drawing.DrawLine(new Vector2(x + 85, y + 21), new Vector2(x - 3, y + 21), 3, VisibleColor[i]);

                            Drawing.DrawLine(new Vector2(x - 5, y + 21), new Vector2(x - 5, y - 4.5f), 3, VisibleColor[i]);
                    ...
                    if (!Menu.Item("showEnemies").GetValue<bool>())
                        continue;
            ...
            {
                Console.WriteLine("Error in SharedExperience.OnDraw + " + ex);
            }
        }
        ...
        private void OnUpdate(EventArgs args)
        {
            try
            {
                ...
                foreach (Obj_AI_Hero hero in ObjectManager.Get<Obj_AI_Hero>())
                {
                    ...
                    if (!hero.IsVisible)
                    {
                        ...
                        if (t <= 4000)
                            MissingColor[i] = Color.FromArgb(255, 255, 80, 0);
                        else if (t <= 6000)
                            MissingColor[i] = Color.FromArgb(255, 255, 120, 0);
                        else if (t <= 8000)
                            MissingColor[i] = Color.FromArgb(255, 255, 150, 0);
                        else if (t <= 10000)
                            MissingColor[i] = Color.FromArgb(255, 255, 180, 0);
                        else if (t <= 12000)
                            MissingColor[i] = Color.FromArgb(255, 255, 210, 0);
                        else if (t <= 14000)
                            MissingColor[i] = Color.FromArgb(255, 225, 200, 0);
                        else if (t <= 16000)
                            MissingColor[i] = Color.FromArgb(255, 200, 190, 0);
                        else if (t <= 18000)
                            MissingColor[i] = Color.FromArgb(255, 180, 180, 0);
                        else if (t <= 20000)
                            MissingColor[i] = Color.FromArgb(255, 150, 190, 0);
                        else if (t > 22000)
                            MissingColor[i] = Color.FromArgb(255, 100, 200, 0);

                        if (Environment.TickCount - TimeMissing[i] >= 5000)
                        {
                            if (IsNearMe[i] == 1 && (Vector3.Distance(hero.Position, MyPos) > 5500))
                                TimeMissing[i] = Environment.TickCount + 30000;
                    ...
                    else
                    {
                        ...
                        if (t <= 4000)
                            VisibleColor[i] = Color.FromArgb(255, 255, 80, 0);
                        else if (t <= 6000)
                            VisibleColor[i] = Color.FromArgb(255, 255, 120, 0);
                        else if (t <= 8000)
                            VisibleColor[i] = Color.FromArgb(255, 255, 150, 0);
                        else if (t <= 10000)
                            VisibleColor[i] = Color.FromArgb(255, 255, 180, 0);
                        else if (t <= 12000)
                            VisibleColor[i] = Color.FromArgb(255, 255, 210, 0);
                        else if (t <= 14000)
                            VisibleColor[i] = Color.FromArgb(255, 225, 200, 0);
                        else if (t <= 16000)
                            VisibleColor[i] = Color.FromArgb(255, 200, 190, 0);
                        else if (t <= 18000)
                            VisibleColor[i] = Color.FromArgb(255, 180, 180, 0);
                        else if (t <= 20000)
                            VisibleColor[i] = Color.FromArgb(255, 150, 190, 0);
                        else if (t > 22000)
                            VisibleColor[i] = Color.FromArgb(255, 100, 200, 0);

                        if (hero.IsDead)
                            MissingColor[i] = Color.FromArgb(255, 200, 200, 200);
                    }

                    if (hero.IsDead || hero.IsMe || (hero.Level == 18) || hero.IsInvulnerable)
                        continue;
                    ...
                    foreach (Obj_AI_Minion minion in ObjectManager.Get<Obj_AI_Minion>())
                    {
                        if (minion.IsAlly && Vector3.Distance(hero.Position, minion.Position) <= 1400)
                        {
                            ...
                            string MinionName = minion.CharData.BaseSkinName;
                        }
                    }
            ...
            {
                Console.WriteLine("Error in SharedExperience.OnUpdate + " + ex);
            }
        }
    }
}