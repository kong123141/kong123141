            ...
            foreach (var cObject in skillshot.SpellData.CollisionObjects)
            {
                switch (cObject)
                {
                    case CollisionObjectTypes.Minion:
                        ...
                        if (!Pluging.Evade.Menu.Item("MinionCollision").GetValue<bool>())
                        {
                            break;
                        }
                        ...
                        {
                            var pred = FastPrediction(from, minion, Math.Max(0, skillshot.SpellData.Delay - (Utils.TickCount - skillshot.StartTick)), skillshot.SpellData.MissileSpeed);
                    ...
                    case CollisionObjectTypes.Champions:
                        if (!Pluging.Evade.Menu.Item("HeroCollision").GetValue<bool>())
                        {
                            break;
                        }
                        ...
                        {
                            var pred = FastPrediction(from, hero, Math.Max(0, skillshot.SpellData.Delay - (Utils.TickCount - skillshot.StartTick)), skillshot.SpellData.MissileSpeed);
                            var pos = pred.PredictedPos;
                    ...
                    case CollisionObjectTypes.YasuoWall:
                        if (!Pluging.Evade.Menu.Item("YasuoCollision").GetValue<bool>())
                        {
                            break;
                        }

                        if (!ObjectManager.Get<Obj_AI_Hero>().Any(hero => hero.IsValidTarget(float.MaxValue, false) && hero.Team == ObjectManager.Player.Team && hero.ChampionName == "Yasuo"))
                        {
                            break;
                        }
                        ...
                        if (intersections.Count > 0)
                        {
                            ...
                            var collisionT = Utils.TickCount + Math.Max(0, skillshot.SpellData.Delay - (Utils.TickCount - skillshot.StartTick)) + 100 + (1000 * intersection.Distance(from)) / skillshot.SpellData.MissileSpeed;
