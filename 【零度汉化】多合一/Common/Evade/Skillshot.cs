    ...
    public class Skillshot
    {
        ...
        public Vector2 CollisionEnd
        {
            get
            {
                ...
                if (IsGlobal)
                {
                    return GlobalGetMissilePosition(0) + Direction * SpellData.MissileSpeed * (0.5f + SpellData.Radius * 2 / ObjectManager.Player.MoveSpeed);
                }
        ...
        {
            return Program.Menu.Item(name + SpellData.MenuItemName).GetValue<T>();
        }
        ...
        public bool Evade()
        {
            ...
            if (!GetValue<bool>("IsDangerous") && Program.Menu.Item("OnlyDangerous").GetValue<KeyBind>().Active)
            {
                _cachedValue = false;
        ...
        {
            if (SpellData.CollisionObjects.Count() > 0 && SpellData.CollisionObjects != null && Utils.TickCount - _lastCollisionCalc > 50 && Program.Menu.Item("EnableCollision").GetValue<bool>())
            {
        ...
        public bool IsSafeToBlink(Vector2 point, int timeOffset, int delay = 0)
        {
            ...
            if (IsSafe(Pluging.Evade.PlayerPosition))
            {
                return true;
            }
            ...
            if (SpellData.Type == SkillShotType.SkillshotMissileLine)
            {
                ...
                var myPositionProjection = Pluging.Evade.PlayerPosition.ProjectOn(Start, End);
                ...
                if (IsSafe(Pluging.Evade.PlayerPosition))
                {
                    ...
                    for (var i = 0; i <= allIntersections.Count - 1; i = i + 2)
                    {
                        ...
                        if (i == allIntersections.Count - 1)
                        {
                            ...
                            return new SafePathResult((End.Distance(missilePositionOnIntersection) + 50 <= End.Distance(enterIntersectionProjection)) && ObjectManager.Player.MoveSpeed < SpellData.MissileSpeed, allIntersections[0]);
                        }
