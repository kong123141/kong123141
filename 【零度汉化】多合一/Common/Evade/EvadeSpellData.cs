namespace Flowers_Utility.Common.Evade
{
    ...
    internal class EvadeSpellData
    {
        ...
        public int DangerLevel
        {
            get
            {
                if (Pluging.Evade.Menu.Item("DangerLevel" + Name) != null)
                {
                    return Pluging.Evade.Menu.Item("DangerLevel" + Name).GetValue<Slider>().Value;
                }
                return _dangerLevel;
            }
        }
        ...
        public bool Enabled
        {
            get
            {
                if (Pluging.Evade.Menu.Item("Enabled" + Name) != null)
                {
                    return Pluging.Evade.Menu.Item("Enabled" + Name).GetValue<bool>();
                }
                return true;
            }
        }
        ...
        public bool IsReady()
        {
            return ((CheckSpellName == "" || ObjectManager.Player.Spellbook.GetSpell(Slot).Name == CheckSpellName) && ((IsSummonerSpell && ObjectManager.Player.Spellbook.CanUseSpell(Slot) == SpellState.Ready) || (!IsSummonerSpell && ObjectManager.Player.Spellbook.CanUseSpell(Slot) == SpellState.Ready)));
        }
    }
