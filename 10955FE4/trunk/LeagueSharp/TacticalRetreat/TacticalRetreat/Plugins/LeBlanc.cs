...
namespace TacticalRetreat.Plugins
{
    internal class LeBlanc
    {
        ...
        {
            //TODO: Add LeBlanc R logic.
            {new Spell(SpellSlot.Q, 700), EscapeType.Not},
            {new Spell(SpellSlot.W, 600), EscapeType.Dash},
            {new Spell(SpellSlot.E, 950), EscapeType.Enemy},
            {new Spell(SpellSlot.R, 0), EscapeType.Not}
        };
        ...
        {
            rootMenu.AddSubMenu(new Menu("LeBlanc", "LeBlanc"));
        ...
        private static void EscapeSpells()
        {
            foreach (KeyValuePair<Spell, EscapeType> spell in spells.Where(s => s.Value != EscapeType.Not))
            {
                ...
                {
                    Player.IssueOrder(GameObjectOrder.MoveTo, escapePoint);
                    //TODO: Make this more generic; this currently only works because LB has 1 dash (normal W).
                    if (spell.Value == EscapeType.Dash && !LBDashing())
                    {
        ...
        private static bool LBDashing()
        {
            return !ObjectManager.Player.Spellbook.GetSpell(SpellSlot.W).IsReady() || ObjectManager.Player.Spellbook.GetSpell(SpellSlot.W).Name == "leblancslidereturn";
        }
        #region Ally Finding
